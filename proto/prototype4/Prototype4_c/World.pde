class World {
  ...
  void step() {
    ...
    for (Munchkin m : munchkins) {
      ...
      // Resolve conflicts.
      Vector<Munchkin> inArea = world.getMunchkinsInArea((int)m.x, (int)m.y, m.size*3);
      inArea.remove(m); // remove itself from the lot
      
     // Keep only things it can eat.
      Iterator<Munchkin> it = inArea.iterator();
      while (it.hasNext()) {
        Munchkin o = it.next();    
        if (!m.isPredatorOf(o))
          it.remove();
    ...
    Vector<Munchkin> splitted = new Vector<Munchkin>();
    while (it.hasNext()) {
      Munchkin m = it.next();
      if (m.size <= 0)
        it.remove();
