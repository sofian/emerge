...
//  drawObservations();
  ...
  // Compute means.
  int k=0;
  int rawObservationSize = DIM_OBSERVATIONS*2;
  int blockSize = CIRCULAR_BUFFER_SIZE/rawObservationSize;
  float rawObservations[] = new float[rawObservationSize];
  for (int i=0; i<rawObservationSize; i++) {
    float avg = 0;
    for (int j=0; j<blockSize; j++) {
      println( k + " " + circularBuffer.get(k));
      avg += ((Number) circularBuffer.get(k++)).floatValue(); }
    avg /= (float)blockSize;
    rawObservations[i] = avg;
  }
  
  // Compute autocorrelation.
  for (int i=0; i<DIM_OBSERVATIONS; i++) {
    float sumProduct = 0;
    for (int j=0; j<DIM_OBSERVATIONS; j++)
      sumProduct += rawObservations[i+j] * rawObservations[j];
    println(sumProduct);
    observationVec[i] = sumProduct / DIM_OBSERVATIONS;
  }
